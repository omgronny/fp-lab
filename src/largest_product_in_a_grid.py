...
for i in range(len(grid) - 3):
    for j in range(len(grid[i]) - 3):
        products.append(product(grid[i][j:j+4]))
        products.append(product([grid[i + k][j] for k in range(4)]))
        products.append(product([grid[i + k][j + k] for k in range(4)]))

reversed_grid = [list(reversed(row)) for row in grid]
for i in range(len(reversed_grid) - 3):
    for j in range(len(reversed_grid[i]) - 3):
